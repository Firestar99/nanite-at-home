...
impl TextureManager {
	pub fn new(init: &Arc<Init>) -> Arc<Self> {
		...
		.unwrap();
		Arc::new(Self { init, sampler })
	}

	pub async fn upload_texture_from_memory(&self, image_data: &[u8]) -> Result<Arc<ImageView>, ImageError> {
		self.upload_texture(image::load_from_memory(image_data).unwrap()).await
	}

	pub async fn upload_texture(&self, image_data: DynamicImage) -> Result<Arc<ImageView>, ImageError> {
		let init = &self.init;
